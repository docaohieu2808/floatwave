...
**youtube-search.js** — `searchYouTube(query, mode='music')`: mode `music` = YouTube Music catalog via youtubei.js (`yt.music.search(q, {type:'song'})`, lazy Innertube singleton) → songs (artist/album), but these are Art Tracks (audio + square album art, no real video); mode `video` = youtube-sr general video search → real video uploads. (Music mode still falls back to video search if the InnerTube path fails.) Returns array of {id, title, channel, duration, thumbnail}. Errors return {ok:false, error}.
